...
# Inicializa as listas dinâmicas para o mapeamento automático de novos integrantes
nomes_do_dia = NOMES_BASE.copy()
novos_nomes = []

# Varredura prévia de detecção de novos colaboradores e linhas visíveis
if uploaded_file:
    try:
        ...
        for r in range(2, sheet_temp.max_row + 1):
            if sheet_temp.row_dimensions[r].hidden == False:
                u = sheet_temp.cell(row=r, column=13).value # Coluna M (USUARIO)
                if u:
...
# Seleção da data da produtividade para atualizar o e-mail automaticamente
...
# Filtros gerenciais limpos baseados nos nomes do dia
...
if uploaded_file:
    ...
    # CORREÇÃO: Títulos limpos exibindo apenas "EXEMPLARES" e "SKU"
    c1, c2 = st.columns(2)
    with c1:
        st.markdown(f'<div class="card-kpi"><div class="card-title">EXEMPLARES</div><div class="card-value">{total_exemplares:,} un</div><div class="card-sub">Meta Diária: {META_EXEMPLARES:,} un | Atingido: {pct_exemplares:.1%}</div></div>', unsafe_allow_html=True)
        st.progress(min(pct_exemplares, 1.0))
    with c2:
        st.markdown(f'<div class="card-kpi" style="background: linear-gradient(135deg, #0F766E 0%, #14B8A6 100%);"><div class="card-title">SKU</div><div class="card-value">{total_skus:,}</div><div class="card-sub">Meta Diária: {META_SKUS:,} | Atingido: {pct_skus:.1%}</div></div>', unsafe_allow_html=True)
        st.progress(min(pct_skus, 1.0))
        
    st.markdown("<br>", unsafe_allow_html=True)

    # Processamento dos indicadores individuais por colaboradora
    data_gerencial = []
